...
def test_div():
    ...
    assert (k := a // 2) == [1, 2, 3, 4, 5] and k._mod == 12
    assert (k := a / 2) == [1, 2, 3, 4, 5] and k._mod == 6
    ...
    assert (k := a // 6) == [5, 10, 1, 6, 11] and k._mod == 14

    a = ArrayMod([2, 4, 6, 8, 10], 11)
    assert (k := a // 2) == [1, 2, 3, 4, 5] and k._mod == 11

    with pytest.raises(ValueError) as exc_info:
        a / 2
    assert "not divisible" in str(exc_info.value)

    assert (k := a // 3) == [8, 5, 2, 10, 7] and k._mod == 11
...
if __name__ == '__main__':
    ...
    pivot = lambda arr: reduce(lambda i, c: c if (
            gcd(c, arr._mod) == 1 or (g > 1 and gcd(c // g, arr._mod) == 1)
